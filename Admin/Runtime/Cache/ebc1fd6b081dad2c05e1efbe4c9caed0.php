...
<script>
		$(function(){
				$("#add").click(function(){
						$("#typevalue").append('<div class="divvalue" style="margin-left:200px;"><input  type="text"  name="typevalue[]" style="width:300px;"/><input type="hidden" name="vid[]"><a href="javascript:void(0)" class="tip-top" data-original-title="删除">&nbsp;&nbsp;<i class="icon-remove"></i></a></div>');
				})
				...
				$('.tip-top').live('click', function(){
					...
					if(result){
						$(this).parent().remove();
				 		//ajax删除掉规格值
				 		var url = "__URL__/delv";
				 		var vid = $(this).attr("vid");
	
				 		$.get(url,{id:vid},function(data){				
...
<div id="content">
	<!-- 顶部右侧快捷操作按钮 开始 -->
		...
		<div id="breadcrumb">
			...
			<a href="__URL__/lists" title="返回用户管理" class="tip-bottom"> 规格管理</a>
						...
						<div class="widget-content nopadding">
							...
							<form action="__URL__/update/id/<?php echo ($data["id"]); ?>"  method="post" enctype="multipart/form-data" class="form-horizontal">
								...
								<div class="control-group">
									...
									<div class="controls">
										   <input type="text" name="typename" value="<?php echo ($data["typename"]); ?>">
									</div>
								</div>
								<div class="control-group">
									<label class="control-label">规格单位：</label>
									<div class="controls">
										   <input type="text" name="danwei" value="<?php echo ($data["danwei"]); ?>">
									</div>
								</div>
								<div class="control-group" id="typevalue">
									<label class="control-label">规格值：</label>
									<div class="controls" >
									<button id="add" type="button" class="btn btn-success">添加规格值</button>
									</div>
										<?php if(is_array($vdata)): $i = 0; $__LIST__ = $vdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="divvalue" style="margin-left:200px;">
											<input  type="text"  name="typevalue[]" style="width:300px;" value="<?php echo ($vo["typevalue"]); ?>"/>
											<input type="hidden" name="vid[]" value="<?php echo ($vo["id"]); ?>">
											<a href="javascript:void(0)" class="tip-top" data-original-title="删除" vid="<?php echo ($vo["id"]); ?>">&nbsp;<i class="icon-remove"></i></a>
