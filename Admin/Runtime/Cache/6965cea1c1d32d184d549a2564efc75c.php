						...
						<div class="widget-content nopadding">
							<table class="table table-bordered data-table">
								...
								<?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
										<td><input type="checkbox" name="all[]" value="<?php echo ($vo["id"]); ?>"/></td>
										<td><?php echo ($vo["ordernum"]); ?></td>
										<td><?php echo ($vo["rname"]); ?></td>
										<td><?php echo ($vo["dilivery"]); ?></td>
										<td><?php echo ($vo["realprice"]); ?></td>
										<td>
											<?php if($vo["orderstatus"] == 1): ?><font color="green">已确认</font>
													<?php else: ?><font color="red">未确认</font><?php endif; ?>
											<?php if($vo["paystatus"] == 1): ?><font color="green">已付款</font>
													<?php else: ?><font color="red">未付款</font><?php endif; ?>
											<?php if($vo["sendstatus"] == 1): ?><font color="green">已发货</font>
													<?php elseif($vo["sendstatus"] == 2): ?><font color="red">未发货</font>
													<?php elseif($vo["sendstatus"] == 3): ?>
													<font color="green">完成交易</font><?php endif; ?>
											<?php if($vo["sendstatus"] == 1 AND $vo.sendstatus): ?><font color="red">待完成交易。。。</font><?php endif; ?>
										</td>
										<td><?php echo (date("Y-m-d H:i",$vo["sendtime"])); ?></td>
										<td class="taskOptions">
										 <a href="__URL__/showsp/id/<?php echo ($vo["id"]); ?>" class="tip-top" data-original-title="查看"><i class="icon-search"></i></a>&nbsp;&nbsp;&nbsp;
									    	<a href="__URL__/del/id/<?php echo ($vo["id"]); ?>" class="tip-top" data-original-title="删除" onclick="return confirm('确定要删除!!!');"><i class="icon-remove"></i></a>&nbsp;&nbsp;
										</td>
