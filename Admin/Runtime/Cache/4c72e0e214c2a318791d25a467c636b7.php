						...
						<div class="widget-content nopadding">
							<table class="table table-bordered data-table">
								...
								<?php if(is_array($data)): foreach($data as $key=>$v): ?><tr height="30">
										<td>
											<input type="checkbox" name="all[]" value="<?php echo ($v["id"]); ?>"/></td>
										<td><?php echo ($v["gname"]); ?></td>
										<td><?php echo ($v["user_name"]); ?></td>
										<td><?php echo ($v["ip_address"]); ?></td>
										<td><?php echo (date("Y-m-d",$v["add_time"])); ?></td>
									
										<td><?php if($v["status"] == 1): ?><img class="status" cid="<?php echo ($v["id"]); ?>" src="__PUBLIC__/admin/img/yes.gif"/><?php else: ?><img class="status" cid="<?php echo ($v["id"]); ?>" src="__PUBLIC__/admin/img/no.gif"/><?php endif; ?>
										</td>

										<td class="taskOptions">
										    	<a href="__URL__/upda/id/<?php echo ($v["id"]); ?>" class="tip-top" data-original-title="编辑"><i class="icon-pencil"></i></a>&nbsp;&nbsp;
										    	<a href="__URL__/del/id/<?php echo ($v["id"]); ?>" class="tip-top" data-original-title="删除" onclick="return confirm('确定要删除!!!');"><i class="icon-remove"></i></a>
										</td>
