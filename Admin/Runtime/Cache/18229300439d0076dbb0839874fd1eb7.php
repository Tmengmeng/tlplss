...
<div id="content">
	<!-- 顶部右侧快捷操作按钮 开始 -->
		...
		<div id="breadcrumb">
			...
			<a href="__URL__/lists" title="返回用户管理" class="tip-bottom"> 权限管理</a>
			<a href="#" class="current"> 管理员日志</a>
		</div>
		<!-- 顶部面包屑导航 结束 -->
		<!-- 主体区域 -->
			...
			<div class="row-fluid">
				<div class="span12">
					...
					<div class="widget-box">
						...
						<!-- 组件标题区域 -->
						<div class="widget-title"><h5>管理员日志</h5><input style="margin-top:5px;" type="submit" value="清空日志"></div>
						<!-- 组件内容区域 -->
						...
						<?php if(empty($data)): ?><tr>
									<th colspan="6"><h4>该用户没有操作日志</h4></th>
								</tr>
						<?php else: ?>
								<?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
										<td><input type="checkbox" value="<?php echo ($v["id"]); ?>" name="check[]"></td>
										<td><?php echo ($v["id"]); ?></td>
										<td><?php echo ($v["userid"]["username"]); ?></td>
										<td><?php echo ($v["loginfo"]); ?></td>
