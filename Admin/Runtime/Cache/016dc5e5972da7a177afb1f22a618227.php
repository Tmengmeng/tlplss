				...
				<div class="span12">
					...
					<div class="widget-box">
						...
						<div class="widget-content nopadding">
							<!-- 编辑表单 -->
							<form action="__URL__/zupdate"  method="post" enctype="multipart/form-data" class="form-horizontal">
								<div class="control-group">
									<label class="control-label"><strong>什么是转移商品分类?</strong></label>
								</div>
								<div class="control-group">
									<label class="control-label">
										
									</label>
									<div class="controls">(1)在添加商品或者在商品管理中,如果需要对商品的分类进行变更,那么你可以通过此功能,正确管理你的商品分类。
									</div>
								</div><br>
								<div class="control-group">
									<label class="control-label">从此分类：</label>
									<div class="controls">
										<select name="id">
		                                        		<option value="0">顶级分类</option>
				                                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php echo ($vo['id'] == $id ? 'selected' : ''); ?> >
									                		<?php $__FOR_START_22917__=1;$__FOR_END_22917__=$vo["num"];for($i=$__FOR_START_22917__;$i < $__FOR_END_22917__;$i+=1){ ?><span style="color: #cdcdcd;">|-----&nbsp;</span><?php } echo ($vo["cname"]); ?>
									           </option><?php endforeach; endif; else: echo "" ;endif; ?>
    										</select>
									</div>

									<label class="control-label">转移到：</label>
									<div class="controls">
										<select name="zid">
		                                        		<option value="0">顶级分类</option>
				                                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php echo ($vo['id'] == $id ? 'selected' : ''); ?> >
								...
								<div class="form-actions">
									<button type="submit" class="btn btn-success">开始转移</button>
